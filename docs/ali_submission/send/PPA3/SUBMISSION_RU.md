...
**Приложения:** `PPA#3 FIGURES.pdf` (поданные в USPTO, Рис. 1–4) · `PPA3_ARCH_EN.svg` (дополнительная архитектурная схема платформы)

> **Примечание:** Поданные Рис. 1–4 показывают front-zone hardware, offline edge-архитектуру, dual-loop ценообразование/транзакции и workflow индукции. Сменные модули A–G, smart glass (PDLC), умная корзина отходов, вертикальная шина питания и геометрия перекрытия датчиков раскрыты в specification и показаны в **`PPA3_ARCH_EN.svg`** (новый дополнительный рисунок для NPPA №1).

### Рис. 1 — Autonomous Retail Cabinet Front View and Front-Zone Antenna

**Подпись (поданная):** *Camera and front-zone antenna support sensor-fusion anti-theft monitoring.*

| Элемент | Описание |
|---------|----------|
| 105 Биометрическая / age-verification камера | Вверху по центру; поле зрения внутрь шкафа |
| 110 Дисплей | Условия, цены, UI корзины/сессии на правой панели |
| Платёжный интерфейс | NFC, слот карты, логотипы платёжных сетей |
| Smart shelves | Многоуровневые полки с товарами |
| 120 Front-zone antenna region | Линейная **RFID-матрица вдоль переднего края полки** |
| Sensor-fusion (подпись) | Камера + front-zone RFID для anti-theft мониторинга |

**Не показано на поданном Рис. 1** (specification + `PPA3_ARCH_EN.svg`): модули A–G, отсек PDLC smart glass, умная корзина отходов, LED smart edge.

### Рис. 2 — Side Section Showing Electronics Bay and Offline Architecture

**Подпись (поданная):** *Боковой разрез — отсек электроники и offline-capable edge-архитектура.*

| Элемент | Описание |
|---------|----------|
| Main storage volume | Верхний объём шкафа (штриховые линии полок) |
| Electronics bay | Нижний отсек с edge-обработкой |
| 400 Edge controller | Главный процессор; хаб локальных подсистем |
| Local database / cache | Каталог, состояние сессии, кэш правил ценообразования |
| Local transaction buffer | Offline-хранилище при недоступности сети/облака |
| Communication modules | LTE / Wi-Fi / Ethernet |
| Door control | Замок и приводы двери |
| Item counting | Датчики полок / счётчики товаров |
| Cabinet sensors | Среда, температура, tamper и т.д. |
| External network / cloud AI | Пунктир; **«Operation continues when unavailable»** |

**Не показано на поданном Рис. 2:** вертикальная шина питания, ИБП, явный блок pricing engine.

### Рис. 3 — Dual-Loop Control: Background Shrinkage Monitoring and Transaction Processing

**Подпись (поданная):** *Dual-loop control — фоновый мониторинг и обработка транзакций при снятии товара.*

**Левая колонка — непрерывный фоновый мониторинг (цикл):**

1. Считывание веса bin **W(t)**
2. Расчёт усушки **ΔW%** относительно **W₀**
3. Классификация постепенной evaporation-driven усушки
4. Обновление freshness index **FI**
5. Обновление dynamic unit price **P(t)** → возврат к шагу 1

**Правая колонка — транзакционный цикл (снятие товара):**

1. Резкое падение веса
2. Корреляция с vision audit (камера)
3. Подтверждение события снятия
4. Charge = снятый вес × **P(t)**
5. Лог транзакции (сессия пользователя)
6. Данные для exit security verification (вес на выходе vs списание)

**Связь колонок (пунктир):** обновлённые **FI** и **P(t)** передаются в транзакционный цикл.

**Связь с другими рисунками:** Рис. 3 — **динамическое ценообразование по качеству** и **fusion вес + камера при транзакции**. Front-zone RFID anti-theft — на Рис. 1 и 4; полная модульная выдача — в `PPA3_ARCH_EN.svg`.

### Рис. 4 — Induction and Stocking Workflow Linked to Front-Zone Hardware

**Подпись (поданная):** *Workflow индукции и загрузки, связанный с front-zone hardware.*

| Шаг | Действие |
|-----|----------|
| 1 | **Induction station / handheld scanner** |
| 2 | Параллельно: **barcode** · **RFID tag** · **expiration date** |
| 3 | **Associate SKU + expiration with unique RFID identifier** |
| 4 | **Stock items on smart shelves** |
| 5 | **Front-zone antenna array + camera** |
| 6 | **Monitoring during user sessions:** RFID removals · camera hand verify · **sensor-fusion anti-theft** |

**Не нарисовано на Рис. 4 (есть в specification):** задняя **FIFO**-загрузка в модули A–G; чтение на переднем крае при продаже (Рис. 1 + 4 вместе).

### Ссылочные номера (specification ↔ рисунки)

| Ref | Компонент | На поданных Рис. 1–4? |
|-----|-----------|----------------------|
| 100 | Корпус / дверь | Рис. 1 (подразумевается) |
| 105 | Биокамера | Рис. 1 |
| 110 | Дисплей | Рис. 1 |
| 120 | Front registration zone (RFID) | Рис. 1, 4 |
| 130 | Тензодатчики / весовые зоны | Рис. 3; architecture SVG |
| 400 | Edge controller | Рис. 2 |
| 500 | Модули A–G | Architecture SVG + specification |
